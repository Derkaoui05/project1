...
const AnimatedMenuContext = React.createContext(undefined);
export function useAnimatedMenuContext() {
  const context = React.useContext(AnimatedMenuContext);
  if (context === undefined) {
    throw new Error('useAnimatedMenuContext must be used within an AnimatedMenuProvider');
  }
  return context;
}
export function AnimatedMenu({ ...props }) {
...
export function AnimatedMenuButtonToggleIcon({ className, size = 'sm', ...props }) {
...
export function AnimatedMenuButtonLabel({
  ...
  return (
    <div
      className={cn('overflow-hidden shrink-0 inline-grid grid-rows-1 grid-cols-1', className)}
      {...props}
    >
      <AnimatePresence>
        <motion.span
...
export function AnimatedMenuButton({ className, children, ...props }) {
  ...
  return (
    <button
      ...
      {...props}
    >
      <MotionConfig transition={{ duration: 0.35, ease: [0.445, 0.05, 0.55, 0.95] }}>
        {children}
      </MotionConfig>
    </button>
  );
...
export function AnimatedMenuList({ variants = menuListVariants, className, children, ...props }) {
  const { isOpen } = useAnimatedMenuContext();
  return (
    <motion.div
...
export function AnimatedMenuItem({ order = 0, variants = itemVariants, ...props }) {
  return (
    <motion.div
      custom={order}
      variants={variants}
...
export function CloseAnimatedMenu({ className, ...props }) {
  const { setIsOpen } = useAnimatedMenuContext();
  const closeMenu = () => setIsOpen(false);
  return (
    <button
      className={cn('appearance-none bg-none outline-none border-none', className)}
      onClick={closeMenu}
      {...props}
    />
  );
